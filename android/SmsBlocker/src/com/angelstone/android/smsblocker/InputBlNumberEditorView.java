...
public class InputBlNumberEditorView extends Activity implements OnClickListener
{
	...
	public void onClick(View v)
	{
		switch (v.getId())
		{
			case R.id.input_number_view_ok_btn:
			{
				...
				String number = PhoneNumberHelpers.removeNonNumbericChar(String.valueOf(numEt.getText())).trim();
				...
				if (PhoneNumberManager.getIntance(this).isInBlacklist(PhoneNumberHelpers.removeNonNumbericChar(number)))
				{
					if (toastShowWaitHandler.IsAllowShow())
					{
				...
				*/
			    ...
			    ((InputMethodManager)getSystemService(INPUT_METHOD_SERVICE)).hideSoftInputFromWindow(numEt.getWindowToken(), InputMethodManager.HIDE_NOT_ALWAYS);
