...
public class SmsBlocker {
	...
	public static boolean SMSblock(Intent intent, Context context) { 

		PhoneNumberManager db = PhoneNumberManager.getIntance(context);
		
		if (db.readSetting("Firewall_switch") 
			&& db.readSetting("sms_reject_switch")) 
		{		
			...
			PhoneNumberDisposition disp = PhoneNumberManager.getIntance(context).queryAction(sender);
	...
	private static void WriteToLog(String smsBody, String smsNumber, Context context)
	{
		smsNumber = Delete86String(smsNumber);
		EventLog evt = new EventLog(PhoneNumberUtils.formatNumber(smsNumber), EventLog.LOG_TYPE_SMS );		
		evt.setSmsTxt(smsBody);
		String tag = PhoneNumberManager.getIntance(context).getTagByNumber(smsNumber);
