...
public class AddFromContactView extends Activity implements OnItemClickListener,
															OnClickListener
{
    ...
    protected void onCreate(Bundle savedInstanceState) 
	{
		...
		Cursor cur = getContentResolver().query(SysCompat.PHONE_URI, null,
				                                 null, null, "UPPER("+SysCompat.COLUMN_PHONE_NAME + ") ASC");
		...
		try
		{
			...
			if (cur == null || cur.getCount() <= 0)
			{
				...
				{
					Toast.makeText(AddFromContactView.this, 
			...
			mNumberColIndex = cur.getColumnIndex(SysCompat.COLUMN_PHONE_NUMBER);
			mDisplayNameColIndex = cur.getColumnIndex(SysCompat.COLUMN_PHONE_NAME);
			//mNameColIndex = cur.getColumnIndex(ContactsContract.RawContacts.ACCOUNT_NAME);
			...
			{		
				...
				map.put("number", PhoneNumberUtils.formatNumber(cur.getString(mNumberColIndex)));
				...
				if (isInBlacklist(PhoneNumberHelpers.removeNonNumbericChar(cur.getString(mNumberColIndex))))
				{
					map.put("checkImg", R.drawable.btn_check_off_disable);
					mCheckState[i] = PhoneNumberHelpers.CHECK_DISABLE;
				}
				else if (containsNumber(PhoneNumberHelpers.removeNonNumbericChar(cur.getString(mNumberColIndex))) != -1)
				{
			...
			Collections.sort(mList, new Comparator<Map<String, Object>>() 
			{
				...
				{
					return c.compare(object1.get("display_name"), object2.get("display_name"));
				}
			...
			final ListView listView = (ListView)this.findViewById(R.id.contact_list);
			
			SimpleAdapter adapter= new SimpleAdapter(this, 
					                                 mList,
		      	 									 R.layout.contacts_row,
											      	 new String[] {"display_name", "number", "checkImg"},
											      	 new int[] {R.id.name, R.id.phonenumber, R.id.contact_row_check_img});
		...
		catch (Exception e)
		{
			...
			Log.d("scfw", "AddFromContactView:" + e.getClass().toString() + e.getLocalizedMessage());
		}
	}
	...
	public void onItemClick(AdapterView<?> parent, View v, int position, long id)
	{
		...
		String name = (String)mList.get(position).get("display_name");
		...
		name = name.replace("-", "");
		name = Delete86String(name);
		
		Intent intent = new Intent();   		       
	    intent.putExtra("contact_result", number); 

	    if (!number.equals(name))
	    {
	    	intent.putExtra("contact_return_name", name);
	    }
	    else
	    {
	    	intent.putExtra("contact_return_name", "");
	    }
	    setResult(3, intent);     
	    finish(); 
		*/
		Map<String, Object> item = (Map<String, Object>)mList.get(position);
		String number = PhoneNumberHelpers.removeNonNumbericChar((String)item.get("number"));
