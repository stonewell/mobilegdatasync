...
public class SelectedNumberListView extends Activity implements OnClickListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		ListView listView = (ListView) this
				.findViewById(R.id.selected_number_list);
		...
		case R.id.selected_number_list_view_delete_btn: {
			...
			for (int i = 0; i < listView.getCount(); i++) {
				if (listView.isItemChecked(i)) {
					Map<String, String> item = (Map<String, String>) mList
							.get(i);
					mDeleteNumbers.add(PhoneNumberHelpers
							.removeNonNumbericChar(item.get("number")));
				}
			}
					...
					.setPositiveButton(android.R.string.ok,
							new DialogInterface.OnClickListener() {
								public void onClick(DialogInterface dialog,
									...
									String[] numbers = (String[]) mDeleteNumbers
											.toArray(new String[mDeleteNumbers
													.size()]);
