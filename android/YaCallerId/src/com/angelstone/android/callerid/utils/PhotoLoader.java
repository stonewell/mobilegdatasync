...
public class PhotoLoader implements Callback {
	...
	/**
	 * A gate to make sure we only send one instance of MESSAGE_PHOTOS_NEEDED at a
	 * time.
	 */
	...
	private static final String[] COLUMNS = new String[] { CallerId.COL_ID,
			CallerId.COL_DATA };

	private final class PhotoContentObserver extends ContentObserver {
		private Long mId;

		public PhotoContentObserver(long id) {
		...
		public void onChange(boolean selfChange) {
			...
			if (mBitmapCache.containsKey(mId)) {
	 ...
	 *          contact
	 */
	public PhotoLoader(Context context, int defaultResourceId) {
	...
	/**
	 * Load photo into the supplied image view. If the photo is already cached, it
	 * is displayed immediately. Otherwise a request is sent to load the photo
	 * from the database.
	 */
	...
	private boolean loadCachedPhoto(ImageView view, long photoId) {
		...
		if (holder == null) {
			...
			// Null bitmap reference means that database contains no bytes for the
			// photo
	...
	public void stop() {
		...
		Enumeration<BitmapHolder> it = mBitmapCache.elements();
	...
	private void cacheBitmap(long id, byte[] bytes) {
		...
		holder.observer = new PhotoContentObserver(id);

		mContext.getContentResolver().registerContentObserver(
				ContentUris.withAppendedId(mCallerIdManager.getContentUri(), id), true,
				holder.observer);
	...
	private void obtainPhotoIdsToLoad(ArrayList<Long> photoIds,
		...
		/*
		 * Since the call is made from the loader thread, the map could be changing
		 * during the iteration. That's not really a problem: ConcurrentHashMap will
		 * allow those changes to happen without throwing exceptions. Since we may
		 * miss some requests in the situation of concurrent change, we will need to
		 * check the map again once loading is complete.
		 */
	...
	private class LoaderThread extends HandlerThread implements Callback {
		...
		/**
		 * Receives the above message, loads photos and then sends a message to the
		 * main thread to process them.
		 */
		public boolean handleMessage(Message msg) {
		...
		private void loadPhotosFromDatabase() {
			...
			mStringBuilder.append(CallerId.COL_ID + " IN(");
			...
			Cursor cursor = null;
			try {
				cursor = mResolver.query(mCallerIdManager.getContentUri(), COLUMNS,
