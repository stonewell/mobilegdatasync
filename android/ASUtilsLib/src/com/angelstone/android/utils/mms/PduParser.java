...
public class PduParser {
	...
	protected PduHeaders parseHeaders(ByteArrayInputStream pduDataStream) {
		...
		while (keepParsing && (pduDataStream.available() > 0)) {
			...
			case PduHeaders.STORE_STATUS:
				 ...
				 * M-Mbox-Delete.conf and M-Delete.conf PDU. For now we ignore this
				 * fact, since we do not support these PDUs
				 */
			...
			case PduHeaders.REPLY_APPLIC_ID:
				/**
				 * The next three header fields are email addresses as defined in
				 * RFC2822, not including the characters "<" and ">"
				 */
			...
			case PduHeaders.PREVIOUSLY_SENT_DATE: {
				/*
				 * Previously-sent-date-value = Value-length Forwarded-count-value
				 * Date-value
				 */
				/* parse value-length */
				parseValueLength(pduDataStream);
			...
			case PduHeaders.MM_FLAGS: {
				...
				/*
				 * not store this header filed in "headers", because now PduHeaders
				 * doesn't support it
				 */
				break;
			}
			...
			case PduHeaders.MBOX_QUOTAS: {
				...
				/*
				 * not store these headers filed in "headers", because now PduHeaders
	...
	protected static int parseUnsignedInt(ByteArrayInputStream pduDataStream) {
		/**
		 * From wap-230-wsp-20010705-a.pdf The maximum size of a uintvar is 32 bits.
	...
	protected static byte[] parseWapString(ByteArrayInputStream pduDataStream,
		...
		assert (null != pduDataStream);
		/**
		 * From wap-230-wsp-20010705-a.pdf Text-string = [Quote] *TEXT End-of-string
		 ...
		 * The Quote is not part of the contents. Quote = <Octet 127> End-of-string
		 * = <Octet 0>
		 * 
		...
		/**
		 * Return *TOKEN or *TEXT (Text-String without QUOTE, Quoted-String without
		 * QUOTED_STRING_FLAG and without End-of-string)
		 */
...
/**
	...
	protected static boolean isText(int ch) {
		 ...
		 * control character (octets 0 - 31) and DEL (127)> LWS = [CRLF] 1*( SP | HT
		 * ) CRLF = CR LF CR = <US-ASCII CR, carriage return (13)> LF = <US-ASCII
		 * LF, linefeed (10)>
		 */
	...
	protected static int parseShortInteger(ByteArrayInputStream pduDataStream) {
		/**
		 * From wap-230-wsp-20010705-a.pdf Short-integer = OCTET Integers in range
		 * 0-127 shall be encoded as a one octet value with the most significant bit
		 * set to one (1xxx xxxx) and with the value in the remaining least
		 * significant bits.
		 */
		assert (null != pduDataStream);
		int temp = pduDataStream.read();
	...
	protected static long parseLongInteger(ByteArrayInputStream pduDataStream) {
		 ...
		 * Multi-octet-integer Multi-octet-integer = 1*30 OCTET The content octets
		 * shall be an unsigned integer value with the most significant octet
		 * encoded first (big-endian representation). The minimum number of octets
		 * must be used to encode the value. Short-length = <Any octet 0-30>
		 */
		assert (null != pduDataStream);
		int temp = pduDataStream.read();
		assert (-1 != temp);
	...
	/**
	 * Parse content type parameters. For now we just support four parameters used
	 * in mms: "type", "start", "name", "charset".
	 * 
	 * @param pduDataStream
	 *          pdu data input stream
	 * @param map
	...
	protected static void parseContentTypeParams(
		 ...
		 * well-known parameter Well-known-parameter-token = Integer-value the code
		 * values used for parameters are specified in the Assigned Numbers appendix
		 * Typed-value = Compact-value | Text-value In addition to the expected
		 * type, there may be no value. If the value cannot be encoded using the
		 * expected type, it shall be encoded as text. Compact-value = Integer-value
		 * | Date-value | Delta-seconds-value | Q-value | Version-value | Uri-value
		 * Untyped-parameter = Token-text Untyped-value the type of the value is
		 * unknown, but it shall be encoded as an integer, if that is possible.
		...
		while (0 < lastLen) {
			...
			switch (param) {
			/**
			 * From rfc2387, chapter 3.1 The type parameter must be specified and its
			 * value is the MIME media type of the "root" body part. It permits a MIME
			 * user agent to determine the content-type without reference to the
			 * enclosed body part. If the value of the type parameter and the root
			 * body part's content-type differ then the User Agent's behavior is
			 * undefined.
			 * 
			 * From wap-230-wsp-20010705-a.pdf type = Constrained-encoding
			 * Constrained-encoding = Extension-Media | Short-integer Extension-media
			 * = *TEXT End-of-string
			 */
			case PduPart.P_TYPE:
			 ...
			 * content-ID of the compound object's "root". If not present the "root"
			 * is the first body part in the Multipart/Related entity. The "root" is
			 * the element the applications processes first.
			 * 
			...
			/**
			 * From oma-ts-mms-conf-v1_3.pdf In creation, the character set SHALL be
			 * either us-ascii (IANA MIBenum 3) or utf-8 (IANA MIBenum 106)[Unicode].
			...
			/**
			 * From oma-ts-mms-conf-v1_3.pdf A name for multipart object SHALL be
			 * encoded using name-parameter for Content-Type header in WSP multipart
			 * headers.
			 * 
	...
	protected static byte[] parseContentType(ByteArrayInputStream pduDataStream,
		...
		/**
		 * From wap-230-wsp-20010705-a.pdf Content-type-value = Constrained-media |
		 * Content-general-form Content-general-form = Value-length Media-type
	...
	protected static boolean parsePartHeaders(ByteArrayInputStream pduDataStream,
		...
		/**
		 * From oma-ts-mms-conf-v1_3.pdf, chapter 10.2. A name for multipart object
		 * SHALL be encoded using name-parameter for Content-Type header in WSP
		 * multipart headers. In decoding, name-parameter of Content-Type SHALL be
		 * used if available. If name-parameter of Content-Type is not available,
		 * filename parameter of Content-Disposition header SHALL be used if
		 * available. If neither name-parameter of Content-Type header nor filename
		 * parameter of Content-Disposition header is available, Content-Location
		 * header SHALL be used if available.
		 * 
		 * Within SMIL part the reference to the media object parts SHALL use either
		 * Content-ID or Content-Location mechanism [RFC2557] and the corresponding
		 * WSP part headers in media object parts contain the corresponding
		 * definitions.
		 */
		int startPos = pduDataStream.available();
		int tempPos = 0;
		...
		while (0 < lastLen) {
			...
			if (header > TEXT_MAX) {
				...
				case PduPart.P_CONTENT_ID:
					/**
					 * From wap-230-wsp-20010705-a.pdf, chapter 8.4.2.21 Content-ID-value
					 * = Quoted-string
					 */
				...
				case PduPart.P_CONTENT_DISPOSITION:
					 ...
					 * Form-data = <Octet 128> Attachment = <Octet 129> Inline = <Octet
					 * 130>
					 */
