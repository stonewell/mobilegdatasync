...
public class GDataXmlUtils {

	public static void defineCharacterEntity(XmlPullParser xpp)
			throws IOException, NumberFormatException, XmlPullParserException {
		for (String name : LATIN1_ENTITIES.keySet()) {
			String charCode = LATIN1_ENTITIES.get(name);
			xpp.defineEntityReplacementText(name,
					String.valueOf((char) Integer.parseInt((String) charCode)));
		}
	}

	public static boolean isEnd(int event) {
		return event == XmlPullParser.END_DOCUMENT;
	}

	public static boolean isEnd(XmlPullParser xpp, String tagName)
			throws XmlPullParserException {
		return isEnd(xpp.getEventType())
				|| ((xpp.getEventType() == XmlPullParser.END_TAG) && (xpp
						.getName().equals(tagName)));
	}
	...
	public static String parseTextAttribute(XmlPullParser xmlParser,
			String nodeName, String attributeName) throws IOException,
			XmlPullParserException {
		xmlParser.next();

		while (!isEnd(xmlParser.getEventType())) {
			String nextName = xmlParser.getName();
			int type = xmlParser.getEventType();

			if ((type == XmlPullParser.END_TAG) && nodeName.equals(nextName)) {
				// end tag found
				return null;
			}

			if (type == XmlPullParser.START_TAG && nodeName.equals(nextName)) {
				String text = xmlParser.getAttributeValue(null, attributeName);
				return text;
			}
			xmlParser.next();
		}
	...
	public static String parseTextNode(XmlPullParser xmlParser, String name)
			throws IOException, XmlPullParserException {
		xmlParser.next();
		while (!isEnd(xmlParser.getEventType())) {
			String nextName = xmlParser.getName();
			int type = xmlParser.getEventType();
			if (type == XmlPullParser.END_TAG && name.equals(nextName)) {
				// end tag found
				return null;
			}
	...
	private static Hashtable<String, String> LATIN1_ENTITIES = new Hashtable<String, String>();
